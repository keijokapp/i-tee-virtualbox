...
import vmAcpiPowerButtonService from './services/vmAcpiPowerButton';
...
router.get('/machine', asyncMiddleware(async (req, res) => {
	...
	if(wantDetails || wantIps) {
		const promises = machines.map(async ({ uuid }, i) => {
			try {
				if(wantDetails) {
					const info = await vmInfoService(uuid);
					for(const j in info) {
						machines[i][j] = info[j];
					}
				}
				...
				if(wantIps) {
					machines[i].ip = await vmIpService(uuid);
				}
			} catch(e) {
		...
		if('networks' in req.body) {
			for(let i = 0; i < req.body.networks.length; i++) {
				...
				if(typeof network === 'string') {
					await vmNetworkService(name, i, 'intnet', network);
				} else {
					await vmNetworkService(name, i, network.type, network.name);
				}
			}
		}
		...
		if('state' in req.body) {
			switch(req.body.state) {
				...
				case 'acpipowerbutton':
					await vmAcpiPowerButtonService(name, false);
					break;
				default:
